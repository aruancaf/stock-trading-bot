...
def get_historical_data(
        ticker_symbol: str,
        time_period: str,
        time_interval: str) -> yf.Ticker:
    return yf.Ticker(ticker_symbol).history(
        period=time_period, interval=time_interval)
...
def get_current_stock_data(ticker_symbol: str) -> {}:
    ...
    stock_data['SMA'] = util.calculate_sma(historical_stock_data)[
        0]  # method broken because of history access
...
def get_price_slope(ticker_symbol: str):  # refactor maybe
    ...
    stock_price_by_time = []
    for i in range(-n, 0):
        stock_price_by_time.append(
            historical_stock_data.iloc[i].to_dict()['Close'])
...
def get_volume_slope(ticker_symbol: str):  # refactor maybe
    ...
    for i in range(-n, 0):
        stock_volume_by_time.append(
            historical_stock_data.iloc[i].to_dict()['Volume'])
