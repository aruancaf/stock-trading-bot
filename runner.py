# make buy sell distinction between stocks purchased for daytrading and
# long term purchases
    ...
    for stock_ticker in stocks:  # purchases stocks based on daytrading patterns
        try:
            ...
            if stock_score >= 0.2 and stock_ticker not in all_active_positions.keys():
                # todo: calculate order amount
                alpaca.create_order(stock_ticker, 1)
                active_positions_to_check[stock_ticker] = sdg.get_current_stock_data(stock_ticker)[
                    'Close']
                all_active_positions[stock_ticker] = sdg.get_current_stock_data(stock_ticker)[
                    'Close']
                print("Based on daytrading pattern analysis, buying",
                      stock_ticker, "Stock Score: ", stock_score)
...
def stock_position_analyzer():
    while True:
        for position in active_positions_to_check.keys():
            threading.Thread(target=check_perform_sell, args=(
                position, active_positions_to_check[position])).start()
...
def check_perform_sell(stock_ticker, purchase_price):
    while True:
        ...
        price_change_percent = util.calculate_price_change(
            current_stock_price, all_active_positions[stock_ticker])
        print(
            "Checking",
            stock_ticker,
            "Gains/Losses",
            ...
            "Price: $",
            current_stock_price)
        if sa.moving_average_checker(stock_ticker) < 0 or price_change_percent <= - \
                const.MAX_STOP_LOSS_PERCENT or sa.volume_checker(stock_ticker) < 0:
    ...
    while True:
        try:
            ...
            if current_time > const.STOCK_MARKET_OPEN_TIME and current_time < const.STOCK_MARKET_CLOSE_TIME:
                ...
                partitioned_stocks = util.partition_array(
                    active_stocks, const.STOCK_SCANNER_PARTITION_COUNT)
                for partition in partitioned_stocks:
                    threading.Thread(
                        target=daytrading_stock_analyzer,
                        args=[partition]).start()
            else:
                alpaca.sell_all_positions()
                print("Market Close")
                for stock_ticker in const.STOCKS_TO_CHECK:  # purchases stocks based on news info
                    threading.Thread(
                        target=news_stock_analyzer, args=(
                            stock_ticker,)).start()
                time.sleep(360000)
        except Exception as e:
            print("Restarting")
